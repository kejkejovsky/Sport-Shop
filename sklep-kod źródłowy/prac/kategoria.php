...
<?php
	...
	if(isSet($_POST['dodaj_producent']))
	{
		...
		if(mysqli_query($baza, $query)) {
			if($item == 0)
				echo "<div class='alert alert-success alert-dissmissible'><a href='.' class='close' data-dismiss='alert' aria-label='close'>&times;</a>Dodano kategorie</div>";
			else
				echo "<div class='alert alert-success alert-dissmissible'><a href='.' class='close' data-dismiss='alert' aria-label='close'>&times;</a>Zmodyfikowano kategorie</div>";
		}else{
			if(mysqli_errno($baza)==1062){
				echo "<div class='alert alert-danger alert-dissmissible'><a href='.' class='close' data-dismiss='alert' aria-label='close'>&times;</a>Kategoria już istnieje</div>";
			}
		}
	}
	//Usuwanie
	if(isSet($_GET['del']))
	{
		...
		if(mysqli_query($baza, $query)) {
			echo "<div class='alert alert-success alert-dissmissible'><a href='.' class='close' data-dismiss='alert' aria-label='close'>&times;</a>Usunięto kategorie</div>";
		}
	}
?>
...
<?php	
	...
	while($rowa = mysqli_fetch_array($zap1)) 
	{
		...
		echo "<td><a href='kategoria.php?id=$rowa[0]'>Edycja</a></td><td>";
		if($numberRows == 0)
			echo "<a href='kategoria.php?del=$rowa[0]'>Usuń</a>";
		echo '</td></tr>';
		$lp++;
	}
?>
...
<?php
	if(isSet($_GET['id'])) 
		...
		echo "<tr><td></td><td>
				<input name='prod_nazwa' class='form-control mr-sm-2' type='text'  placeholder='Nazwa kategorii' value='$name' required /></td>
				<td>
